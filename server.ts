...
export function app(): express.Express {
  ...
  server.use((req, res, next) => {
    // User Mobile Detact to set Ionic config depending on the device
    const mobileDetect = new MobileDetect(req.headers['user-agent']);
    
    // This method returns the detacted opearting system string or null
  ...
  APP_ROUTES.forEach(route => {
    ...
    server.get(`${route}`, (req, res) => {
      // tslink:disable-next-line: no-console
